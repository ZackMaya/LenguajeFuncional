...
;(caadr (cdr l2))  ---> o (caaddr l2) -------------------->100
;(cadadr (cdr l2))   --------------------------------------> 3
;(car(cadddr(caddr l2)))  ---------------------------------> 5
;(car(cdr(cadddr(caddr l2)))) -----------------------------> 6

(define li '(1 2 3 (4 (5 7)(8 9 (10)))))
;(car (car (cddr (cdr  li))))------------------------------>4
;(car (car (cdr(car (cddr (cdr  li))))))----> o (caadr(cadddr li))-->5
;(car(car(cdr(cdr(car(cddr(car(cdddr li))))))))---> o (caaddr(caddr(cadddr li)))--->10
...
;Cuadrado-lista, dada una lista de enteros
;devuelve una lista con los cuadrados de sus elementos
...
;Reversa de una lista sin concatenar TAREA (invierte-tuneado=reversaAux)
;(define (reversaAux aux xs)
 ; (if (null? xs)
  ;    aux
   ;   (reversaAux(cons(car xs) aux)(cdr xs))))


;(reversaAux '(1 2 3 4) '())
;(define reversa (curry reversaAux '()))
;(reversa '(1 2 3 4))

;Versión con Let
;(define (reversa xs)
 ; (let((reversaAux (lambda(ys aux)
  ;                   (if(null? ys)
   ;                     aux
    ;                    (reversaAux(cons(car ys)aux)(cdr ys))))))
    ;(reversaAux xs '())))

;Version con Letrec y lambda
(define (reversa xs)
  (letrec((reversaAux (lambda(ys aux)
                     (if(null? ys)
                        aux
                        (reversaAux(cdr ys)(cons(car ys)aux))))))
    (reversaAux xs '())))

;Funciones de Plegado función FOLDR
(define (mi-foldr f x xs)
  (if (null? xs)
      x
      (f(car xs)(mi-foldr f x (cdr xs)))))

;Miercoles 08/03/2017
;(define (sumal(curry foldr + 0)))
(define (sumal xs)
  (if(null? xs) 0 (+ (car xs)(sumal(cdr xs)))))
(define (sumall xs)
  (if (null? xs)
      0
      (+(sumal (car xs))(sumall(cdr xs)))))

;(define suma_aux )
;(define (sumall_foldr xs)
 ; (foldr 0 suma_aux xs))
(define lis1 '(1 2 3 4))
...
;Jueves 09/03/2017
;Funcion que borra un elemento de una lista
...
;(define (borrar y xs)
 ; (if (null? xs)
  ;    '()
   ;   (if(=(car xs)y)
    ;     (borrar y (cdr xs))
     ;   (cons(car xs)(borrar y (cdr xs))))))

;Función que borra un elemento con foldr
;Foldr hasta el final verifica (se necesitan 3 cosas para foldr...
(define (borrar y xs)
  (let(
      (f (lambda(a ys)
           (if (= a y)
...
;Lo que hace foldr paso a paso
;(borraro 1 '(1 2 1 3))
;(foldr f '() '(1 2 1 3))
;(f 1 (foldr f '() '(2 1 3)))
;(f 1 (f 2 (foldr f '() '(1 3))))
;(f 1 (f 2 (f 1 (foldr f '() '(3)))))
;(f 1 (f 2 (f 1 (f 3 (foldr f '() '())))))

;definir la función que recibe una lista y un elemento y devuelve
;true en caso de que el elemento este en la lista "pertenece"
(define (member? elem list)
   (if (equal? list '())
      #f
      (or (equal? elem (car list)) (member? elem (cdr list)))))

;Versión funcion simple
(define (pertenece y xs)
  (if (null? xs)
             #f
             (if(=(car xs) y)
                #t
               (or #f (pertenece y (cdr xs))))))


;Versión pertenece con foldr




;Examen hasta el foldr
...
(define lista '(1 2 3 (4 (5 7)(8 9 (10)))))

(caadr(cadddr lista));----------------------> 5
(caaddr(caddr(cadddr lista)));--------------> 10


;Grado de un polinomio para exp
(define grado-exp
  (lambda (polinomio)
    (-(length polinomio)1)))


(define (entierra x num)
  (if (= num 0)
      (cons x '())
     (cons(entierra x(- num 1))'())
     )
  )
...
;Para el ejercicio 9 programar un algoritmo de ordenamiento
;Insertion-sort
;Quick-sort
;Merge-sort
;Burbuja
...
(merge-sort '((5 4 3 2 1)(5 4 3 2)(5 4 3)))

;(take '(3 2 5 4 8) 4)
;regresa los n numeros de la lista
;(drop '(3 2 5 4 8) 4)
;Borra los n numeros de la lista
...
(define (multiplo nu xxs)
 (if (null? xxs) '()
     (if(= 0 (remainder(car xxs) nu)) 
        (multiplo nu (cdr xxs))
     (cons (car xxs)(multiplo nu (cdr xxs))))))

(define (criba-aux xs)
       (if (null? xs)
           '()
           (cons(car xs)(criba(multiplo (car xs)xs)))))

(define (criba n)
  (criba-aux (construye_lista_n n)))
